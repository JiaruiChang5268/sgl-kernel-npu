...
def _matmul_kernel(
    ...
    if NUM_BLOCKS_M >= BLOCK_TRESHHOLD and NUM_BLOCKS_N >= BLOCK_TRESHHOLD:
        for block_idx in range(pid, NUM_BLOCKS, num_cores):
            ...
            cur_threshold_m = (
                BLOCK_TRESHHOLD
                if block_idx < (NUM_BLOCKS_M // BLOCK_TRESHHOLD * BLOCK_TRESHHOLD) * NUM_BLOCKS_N
                else NUM_BLOCKS_M % BLOCK_TRESHHOLD
            )
            cur_threshold_m_n = cur_threshold_m * BLOCK_TRESHHOLD
            cur_threshold_n = (
                BLOCK_TRESHHOLD
                if block_idx % (NUM_BLOCKS_N * BLOCK_TRESHHOLD) < (cur_threshold_m * NUM_BLOCKS_N) // cur_threshold_m_n * cur_threshold_m_n
                else NUM_BLOCKS_N % BLOCK_TRESHHOLD
            )
            local_relative_block = (
                block_idx % (BLOCK_TRESHHOLD * NUM_BLOCKS_N) % (BLOCK_TRESHHOLD * cur_threshold_m)
            )
            task_m_idx = (
            ...
            x, y = (
                cur_threshold_m,
                cur_threshold_n if cur_threshold_m > cur_threshold_n else cur_threshold_n,
            )
            while y != 0:
                x, y = y, x % y
            lcm = cur_threshold_m * cur_threshold_n // x
            task_n_idx = (
                (local_relative_block + (local_relative_block // lcm)) % cur_threshold_n
                + block_idx % (BLOCK_TRESHHOLD * NUM_BLOCKS_N) // cur_threshold_m_n * BLOCK_TRESHHOLD
            )
